...
export async function submitGuestOrder(input: {
  ...
  const { orderId, notificationPayload } = await db.transaction(
    async (transactionDb) => {
      ...
      const variants = new Map(
        variantRows.map((row) => [row.variant_id, row] as const),
      );
      let total = 0;
      ...
      for (const item of items) {
        ...
        const activeUnitPrice = getActiveUnitPrice({
          regularAmount: variant.unit_price,
          metadata: parseJsonObject(variant.metadata_json),
        });

        total +=
          convertAmount(activeUnitPrice, variant.currency_code, currencyCode) *
          item.quantity;
      }
      ...
      const displayRow = await transactionDb
        .prepare(
          "select coalesce(max(display_id), 0) as max_display_id from orders",
        )
